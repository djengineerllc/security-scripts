...
openssl genrsa -out $WORKING_DIR/$HOSTNAME.first.key 4096
#Generate new CSR used for let's encrypt to sign
openssl req -new -key $WORKING_DIR/$HOSTNAME.first.key -sha256 -out $HOSTNAME.first.csr -config $WORKING_DIR/openssl.cnf
